...
// import { versionKey } from '../api.config'

const prependUrl = 'http://xsjyule.dividendss.com'
const platformKey = '3LK0V/qWsjnMe935IUgNzw=='

// 登录和注册验证码
export const captcha = `${prependUrl}/qm/user/captcha?platformKey=${platformKey}`
...
// 获取皇家娱乐新活动信息(模式二)
export const queryNewActivity = (data = {}) => {
  return fetch({
    api: '/user/queryNewActivity',
    type: 'get',
    params: data
  })
}
...
// 获取所有app版本信息
export const getMoblieVersion = (data) => {
  return fetch({
...
export const isnoPq = () => {
  return fetch({
    api: '/user/version/getRegisteInfoByPhone',
    type: 'get'
  })
}
...
// 直营模式配置
export const getOperationModel = (data) => {
  return fetch({
    type: 'get',
    api: '/user/getOperationModel',
    params: {terraceKey: versionKey}
  })
}
